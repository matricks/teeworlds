		...
		// load images
		{
			...
			for(int i = 0; i < Num; i++)
			{
				...
				if(pItem->m_External)
				{
					...
					{
						*pImg = ImgInfo;
						pImg->m_Texture = m_pEditor->Graphics()->LoadTextureRaw(aBuf, ImgInfo.m_Width, ImgInfo.m_Height, ImgInfo.m_Format, ImgInfo.m_pData, CImageInfo::FORMAT_AUTO, 0);
						pImg->m_External = 1;
					}
				}
				else
				{
					...
					pImg->m_Texture = m_pEditor->Graphics()->LoadTextureRaw(pName, pImg->m_Width, pImg->m_Height, pImg->m_Format, pImg->m_pData, CImageInfo::FORMAT_AUTO, 0);
				}
