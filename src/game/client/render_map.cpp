									...
									ENVELOPE_EVAL pfnEval, void *pUser, int ColorEnv, int ColorEnvOffset)
{
	if(true)
	{
		RenderTilemapNew(pTiles, w, h, Scale, Color, RenderFlags, pfnEval, pUser, ColorEnv, ColorEnvOffset);
	}
	else
	{
		RenderTilemapOld(pTiles, w, h, Scale, Color, RenderFlags, pfnEval, pUser, ColorEnv, ColorEnvOffset);
	}
}

void CRenderTools::RenderTilemapOld(CTile *pTiles, int w, int h, float Scale, vec4 Color, int RenderFlags,
									ENVELOPE_EVAL pfnEval, void *pUser, int ColorEnv, int ColorEnvOffset)
{
...
void CRenderTools::RenderTilemapNew(CTile *pTiles, int w, int h, float Scale, vec4 Color, int RenderFlags,
									ENVELOPE_EVAL pfnEval, void *pUser, int ColorEnv, int ColorEnvOffset)
{
	//Graphics()->TextureSet(img_get(tmap->image));
	float ScreenX0, ScreenY0, ScreenX1, ScreenY1;
	...
	const int MAX_VERTICES = 1024*4;
	IGraphics::CColor FinalColor = { Color.r*r, Color.g*g, Color.b*b, Color.a*a };
		...
		for(int x = StartX; x < EndX; x++)
		{
			...
			if(Index)
			{
				...
				if(Render)
				{
					...
					aVertices[NumVertices+0].m_Pos.x = sx;			aVertices[NumVertices+0].m_Pos.y = sy;
					aVertices[NumVertices+0].m_Tex.u = x0;			aVertices[NumVertices+0].m_Tex.v = y0;
					aVertices[NumVertices+0].m_Color = FinalColor;

					aVertices[NumVertices+1].m_Pos.x = sx+Scale;	aVertices[NumVertices+1].m_Pos.y = sy;
					aVertices[NumVertices+1].m_Tex.u = x1;			aVertices[NumVertices+1].m_Tex.v = y1;
					aVertices[NumVertices+1].m_Color = FinalColor;

					aVertices[NumVertices+2].m_Pos.x = sx+Scale;	aVertices[NumVertices+2].m_Pos.y = sy+Scale;
					aVertices[NumVertices+2].m_Tex.u = x2;			aVertices[NumVertices+2].m_Tex.v = y2;
					aVertices[NumVertices+2].m_Color = FinalColor;

					aVertices[NumVertices+3].m_Pos.x = sx;			aVertices[NumVertices+3].m_Pos.y = sy+Scale;
					aVertices[NumVertices+3].m_Tex.u = x3;			aVertices[NumVertices+3].m_Tex.v = y3;
					aVertices[NumVertices+3].m_Color = FinalColor;
