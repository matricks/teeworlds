...
void CGameConsole::PossibleCommandsRenderCallback(const char *pStr, void *pUser)
{
	...
	if(pInfo->m_EnumCount == pInfo->m_WantedCompletion)
	{
		...
		pInfo->m_pSelf->Graphics()->TextureSet(0);
...
void CGameConsole::OnRender()
{
	...
	// do background
	Graphics()->TextureSet(g_pData->m_aImages[IMAGE_CONSOLE_BG].m_Resource);
	Graphics()->QuadsBegin();
	...
	// do the lower bar
	Graphics()->TextureSet(g_pData->m_aImages[IMAGE_CONSOLE_BAR].m_Resource);
	Graphics()->QuadsBegin();
