...
void CMenus::RenderHSLPicker(CUIRect Picker)
{
	...
	// Hue/Lgt picker :
	{
		...
		// picker
		Graphics()->TextureSet(g_pData->m_aImages[IMAGE_HLPICKER].m_Resource);
		Graphics()->QuadsBegin();
...
void CMenus::RenderSettingsTee(CUIRect MainView)
{
	...
	// Parts Customization :
	{
		...
		for(int p = 0; p < NUM_SKINPARTS; p++)
		{
			IGraphics::CTextureHandle Tex = g_pData->m_aImages[IMAGE_NO_SKINPART].m_Resource;
			int Sprite = -1;
			if(gs_apSkinVariables[p][0])
			{
				int SkinPart = m_pClient->m_pSkins->FindSkinPart(p, gs_apSkinVariables[p]);
	...
	if(DoButton_CheckBox(&s_ButtonSndEnable, Localize("Use sounds"), g_Config.m_SndEnable, &Button))
	{
		...
		if(g_Config.m_SndEnable)
		{
			if(g_Config.m_SndMusic)
				m_pClient->m_pSounds->Play(CSounds::CHN_MUSIC, m_Music, 1.0f);
		}
		else
