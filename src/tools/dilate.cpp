...
int DilateFile(const char *pInputFilename, const char *pOutputFilename)
{
	png_t Png;
	CPixel *pBuffer[3] = {0,0,0};

	png_init(0, 0);
	png_open_file(&Png, pInputFilename);

	if(Png.color_type != PNG_TRUECOLOR_ALPHA)
	{
		dbg_msg("dilate", "%s: not an RGBA image", pInputFilename);
		return 1;
	}
	...
	// save here
	png_open_file_write(&Png, pOutputFilename);
...
int main(int argc, const char **argv)
{
	...
	if(argc != 3)
	{
		dbg_msg("Usage", "%s INTPUT OUTPUT", argv[0]);
		return -1;
	}

	return DilateFile(argv[1], argv[2]);
}
